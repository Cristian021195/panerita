...
class DetallesModel extends DbModel{
    ...
    public function readOne(){
        ...
        $_id_comprobante = mysqli_real_escape_string($this->connect, $this->id_comprobante);
        $this->db_close();
        $this->query = "SELECT productos.nombre AS producto, sum(cantidad) as cantidad, devolucion, precio, sum(cantidad * precio) as total, detalles.id_producto as id_producto, detalles.fecha as fecha FROM detalles INNER JOIN productos ON detalles.id_producto = productos.id_producto WHERE id_comprobante = $_id_comprobante GROUP BY id_producto";
        //$this->query = "SELECT productos.nombre AS producto, cantidad, devolucion, precio, (cantidad * precio) as total, detalles.id_producto as id_producto, detalles.fecha as fecha FROM detalles INNER JOIN productos ON detalles.id_producto = productos.id_producto WHERE id_comprobante = $_id_comprobante;";
        $this->get_query();
    ...
    public function deleteSimple(){
        ...
        $_id_comprobante = mysqli_real_escape_string($this->connect, $this->id_comprobante);
        $_id_producto = mysqli_real_escape_string($this->connect, $this->id_producto);
        $_fecha = mysqli_real_escape_string($this->connect, $this->fecha);
        $this->db_close();

        $this->query = "DELETE FROM detalles WHERE id_comprobante = '$_id_comprobante' AND id_producto = '$_id_producto' AND fecha = '$_fecha';";
        if($this->set_query()){
            return false;
        }else{
            return true;

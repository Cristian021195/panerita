...
class ProductosModel extends DbModel{
    ...
    public function read(){
        ...
        if($zona == 'super'){
            $this->query = "SELECT id_producto, nombre, base, distribuidor, mayorista, minorista, zona FROM productos ORDER BY id_producto DESC;";
        }else{
            $this->query = "SELECT id_producto, nombre, base, distribuidor, mayorista, minorista, zona FROM productos WHERE zona = '$zona' ORDER BY id_producto DESC;";
        }
        $this->get_query();
    ...
    public function readFilterLike($type){
        $zona = $_SESSION['zona'];
        if($zona == 'super'){
            $this->query = "SELECT id_producto, nombre, $type as precio FROM productos WHERE nombre LIKE '%$this->nombre%' OR id_producto LIKE '%$this->nombre%';";
        }else{
            $this->query = "SELECT id_producto, nombre, $type as precio FROM productos WHERE nombre LIKE '%$this->nombre%' OR id_producto LIKE '%$this->nombre%' AND zona = '$zona';";
        }
        $this->get_query();
        $this->num_rows = count($this->rows);
        return $this->rows;

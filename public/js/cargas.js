...
export function existeCarga(){
    ...
    const $scrollSectionSecond = document.getElementById('scrollSectionSecond');
    const $f_cliente = document.getElementById('f_cliente');
    const $filtrarCargas = document.getElementById('filtrarCargas');
    ...
    const $table_second = document.createElement('table'); $table_second.classList.add('table'); $table_second.classList.add('table-striped');
    ...
    $f_cliente.addEventListener('input', (e)=>{
        let datos = new FormData($filtrarCargas);
    ...
    //DATOS
    /*s$generarCarga.addEventListener('submit', (e)=>{
        e.preventDefault();
    ...
    $comprobantes.addEventListener('click', (e)=>{
        if(comprobantesObj.comprobantes.length == 0 || comprobantesObj.cajones == ''){
            $scrollSectionSecond.scrollTo(0,0);
            $error2.innerHTML = '<div class="alert alert-danger"><b>¡Error! </b>No hay comprobantes o cajones cargados..</div>';
            mensaje($error2);
        }else{
    ...
    function crearCuerpo(cabecera){
        ...
        cabecera.forEach(e=>{
            ...
            <td>${e.total}</td>
            <td><button class="btn btn-success agregar" disabled data-id="${e.id_comprobante}">➕</button></td>
            </tr>`;
            }else{
                $tbody.innerHTML += `<tr>
            <td>${e.fecha}</td>
            ...
            <td><button class="${color} estado" id="${e.id_comprobante}">${e.estado}</button></td>
            <td>${e.total}</td>
            <td><button class="btn btn-success agregar" data-id="${e.id_comprobante}">➕</button></td>
            </tr>`;
            }//<button class="btn remover" data-id="${e.id_comprobante}">❌</button> // <button class="btn remover" data-id="${e.id_comprobante}">❌</button>
            
        });
        return $tbody;
    }

    function crearCabeceraSecond(datos){
        const thead_data = Object.keys(datos);
        let $thead = document.createElement('thead');
        $thead.innerHTML = `<tr>
                <th>${thead_data[0]}</th>
                <th>${thead_data[1]}</th>
    ...
    function crearCuerpoSecond(datos){
        let $tbody = document.createElement('tbody');
        datos.forEach(e=>{            
            $tbody.innerHTML += `<tr>
            <td>${e.fecha}</td>
            <td>${e.cliente}</td>
            <td>${e.vendedor}</td>
            <td>${e.id_comprobante}</td>
            <td>${e.total}</td>
            <td><button class="btn remover" data-id="${e.id_comprobante}">❌</button></td>
            </tr>`;            
        });
        return $tbody;
    }
    ...
    function agregarEventos(){
        ...
        document.querySelector('table').querySelectorAll('.agregar').forEach(btn =>{
            btn.addEventListener('click', (id)=>{
                ...
                if(comprobantesObj.comprobantes.length > 0){
                    $table_second.innerHTML = '';
                    $table_second.appendChild(crearCabeceraSecond(comprobantesObj.comprobantes[0]));
                    $table_second.appendChild(crearCuerpoSecond(comprobantesObj.comprobantes));
                    $sectionSecond.appendChild($table_second);


                    $table_second.querySelectorAll('.remover').forEach(btn=>{
                        btn.addEventListener('click', (id)=>{
                            removerSecond(id.target.dataset.id);
                ...
                //console.log($table_second);
                //crearCabeceraSecond(comprobantesObj.comprobantes);

                /*$table.appendChild(crearCabecera(comprobantesObj.comprobantes));
                $table.appendChild(crearCuerpo(comprobantesObj.comprobantes));
                $sectionSecond.appendChild($table);*/
            });
        });
    ...
    function removerSecond(id){
        comprobantesObj.comprobantes.forEach((comp, index) =>{
            $table.querySelectorAll('.agregar').forEach(td=>{
                if(td.dataset.id == id){
                    td.removeAttribute('disabled');
                }
            })
            if(comp.id_comprobante == id){
